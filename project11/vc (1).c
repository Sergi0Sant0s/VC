...
float *rgb_to_hsv(float r, float g, float b)
{
	...
	/*if (cmax == cmin)
		...
		aux[0] = (360 + 60 * (g - b)) / diff;
	else if (cmax == g)
		aux[0] = (120 + 60 * (b - r)) / diff;
	else if (cmax == b)
		aux[0] = (240 + 60 * (r - g)) / diff;*/
	if (cmax == cmin)
		aux[0] = 0;
	else if (cmax == r)
		aux[0] = fmod((60 * ((g - b) / diff) + 360), 360.0);
	else if (cmax == g)
		aux[0] = fmod((60 * ((b - r) / diff) + 120), 360.0);
	else if (cmax == b)
		aux[0] = fmod((60 * ((r - g) / diff) + 240), 360.0);
...
int vc_rgb_to_hsv(IVC *original, IVC *converted)
{
	...
	for (x = 0; x < original->width; x++)
	{
		...
		{
			pos = y * original->bytesperline + x * original->channels;
			int red = pos, green = red + 1, blue = green + 1;
			aaa = rgb_to_hsv(original->data[red], original->data[green], original->data[blue]);

			//atribuição
			converted->data[red] = (unsigned char)aaa[0];		//array[0];
			converted->data[green] = (unsigned char)aaa[1]; //array[1];
			converted->data[blue] = (unsigned char)aaa[2];	//array[2];
		}
	}
	return 0;
}